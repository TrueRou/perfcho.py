...
async def _rebuild_rankings(runtime: MigrationRuntime) -> None:
    async def handler(session: AsyncSession, rows: list[SourceRow]) -> None:
        ...
        for row in rows:
            ...
            await project_accepted_score(session, event, f"scoreboard:{event.payload['scoreboard_id']}")
            projected += 1
    ...
    async def handler(session: AsyncSession, rows: list[SourceRow]) -> None:
        ...
        for row in rows:
            ...
            try:
                ...
                await session.execute(play_statement.on_conflict_do_nothing())
                ...
                if policy_id is not None:
                    ...
                    await session.execute(ranked_statement.on_conflict_do_nothing())
    ...
    *,
    ...
    release_id = await session.scalar(
        ...
        .values(
            ...
            ruleset=ruleset,
            version=version,
            artifact_digest=hashlib.sha256(f"bancho.py:v5.2.2:{kind.value}:{ruleset.value}".encode()).digest(),
