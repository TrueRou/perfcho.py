...
class Beatmap:
    ...
    def _parse_from_osuapi_resp(self, osuapi_resp: dict[str, Any]) -> None:
        ...
        self.filename = (
            ("{artist} - {title} ({creator}) [{version}].osu").format(**osuapi_resp).translate(IGNORED_BEATMAP_CHARS)
        )
...
class BeatmapSet:
    ...
    async def _update_if_available(self) -> None:
        ...
        if api_data["data"] is not None:
            ...
            await app.state.services.database.execute(
                "REPLACE INTO mapsets (id, server, last_osuapi_check) VALUES (:id, :server, :last_osuapi_check)",
                {
                    "id": self.id,
                    "server": "osu!",
