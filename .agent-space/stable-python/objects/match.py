    ...
    async def await_submissions(
        ...
        for s in was_playing:
            ...
            while True:
                ...
                if rc_score and rc_score.bmap and rc_score.bmap.md5 == self.map_md5 and rc_score.server_time > max_age:
    ...
    async def update_matchpoints(self, was_playing: Sequence[Slot]) -> None:
        ...
        if ffa:
            ...
            if self.winning_pts and wmp == self.winning_pts:
                ...
                msg.append(
                    f"{wname} takes the match, finishing {match_name} with a score of {wmp} - {lmp}! Congratulations!",
                )
            else:
                # no winner, just announce the match points so far.
