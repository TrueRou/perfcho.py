...
from typing import Any, Never, Self, TypeGuard, Union
...
class SbPatcherScoreMetaRawV2:
    ...
    def trim_pauses(self, map_duration_ms: int | None, trim_start_ms: int = 0) -> list[tuple[start, duration]] | None:
        if self.p is None:
            return None
        ...
        return [
            pause
            for pause in self.p
            if pause[0] > trim_start_ms and (pause[0] < map_duration_ms if map_duration_ms is not None else True)
        ]
...
class _SbPatcherScoreMeta:
    ...
    beatmap_meta: Beatmap | None = None
