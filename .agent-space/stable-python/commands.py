...
from app.objects.match import Match, MatchTeams, MatchTeamTypes, MatchWinConditions, SlotStatus
...
async def changename(ctx: Context) -> str | None:
    return "[https://osu.ppy.sb/me/settings User Settings - Guccho]"
...
async def top(ctx: Context) -> str | None:
    ...
    return "\n".join(
        ...
        + [TOP_SCORE_FMTSTR.format(idx=idx + 1, domain=app.settings.DOMAIN, **s) for idx, s in enumerate(scores)],
    )
...
async def _map(ctx: Context) -> str | None:
    ...
    if len(ctx.args) != 2 or ctx.args[0] not in ("rank", "unrank", "love") or ctx.args[1] not in ("set", "map"):
...
async def user(ctx: Context) -> str | None:
    ...
    priv_list = [priv.name for priv in Privileges if player.priv & priv and bin(priv).count("1") == 1][::-1]
    ...
    donator_info = (
        f"True (ends {timeago.format(player.donor_end)})" if player.priv & Privileges.DONATOR != 0 else "False"
    )
...
def ensure_match(
    ...
    async def wrapper(ctx: Context) -> str | None:
        ...
        if not (
            ctx.player in match.refs or ctx.player.priv & Privileges.TOURNEY_MANAGER or f is mp_help.__wrapped__  # type: ignore[attr-defined]
        ):
            return None
...
async def mp_scrim(ctx: Context, match: Match) -> str | None:
    ...
    if winning_pts != 0:
        ...
        msg = f"A scrimmage has been started by {ctx.player.name}; first to {winning_pts} points wins. Best of luck!"
    else:
        # setting to 0
        if not match.is_scrimming:
