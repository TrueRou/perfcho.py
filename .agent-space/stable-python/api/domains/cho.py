...
from app.objects.player import Action, ClientDetails, OsuStream, OsuVersion, Player, PresenceFilter
from app.packets import BanchoPacketReader, BasePacket, ClientPackets, LoginFailureReason
...
async def bancho_handler(
    ...
    if not player:
        ...
        return Response(
            content=(
                app.packets.notification("Server has restarted.") + app.packets.restart_server(0)
...
async def handle_osu_login_request(
    ...
    if not (running_under_wine or any(adapters)):
        return {
            ...
            "response_body": (
                app.packets.login_reply(-1) + app.packets.notification("Please restart your osu! and try again.")
            ),
        }
    ...
    for channel in app.state.sessions.channels:
        if (
            not channel.auto_join or not channel.can_read(player.priv) or channel.real_name == "#lobby"
    ...
    async def handle(self, player: Player) -> None:
        ...
        if target is not app.state.sessions.bot:
            ...
            if target.is_online:
                ...
                player.enqueue(
                    app.packets.notification(
                        f"{target.name} is currently offline, but will receive your messsage on their next login.",
                    ),
                )
...
class MatchComplete(BasePacket):
    async def handle(self, player: Player) -> None:
        ...
        not_playing = [
            s.player.id for s in player.match.slots if s.player is not None and s.status != SlotStatus.complete
        ]
