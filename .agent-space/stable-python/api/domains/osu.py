...
from app.objects.player import Player, Privileges
from app.objects.score import Score, SubmissionStatus
...
async def osuSearchSetHandler(
    ...
    bmapset = await app.state.services.database.fetch_one(
        f"SELECT DISTINCT set_id, artist, title, status, creator, last_update FROM maps WHERE {k} = :v",
        {"v": v},
    )
    if bmapset is None:
        # TODO: get from osu!
...
async def osuSubmitModularSelector(
                ...
                if score.rank == 1 and not score.player.restricted:
                    ...
                    if prev_n1:
                        if score.player.id != prev_n1["id"]:
                            ann.append(
                                f"(Previous #1: [https://{app.settings.DOMAIN}/u/{{id}} {{name}}])".format(
                                    id=prev_n1["id"],
                                    name=prev_n1["name"],
                                ),
                            )
    ...
    log(
        f"[{score.mode!r}] {score.player} submitted a score! ({score.status!r}, {score.pp:,.2f}pp / {stats.pp:,}pp)",
        Ansi.LGREEN,
    )
