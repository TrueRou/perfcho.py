...
async def api_calculate_pp(
    ...
    final_results = [performance_result | {"accuracy": score.acc} for performance_result, score in zip(results, scores)]

    return ORJSONResponse(
        # XXX: change the output type based on the inputs from user
        (final_results if all(x is None for x in [ngeki, nkatu, n100, n50]) else final_results[0]),
...
async def api_search_players(
    ...
    rows = await app.state.services.database.fetch_all(
        "SELECT id, name FROM users WHERE name LIKE COALESCE(:name, name) AND priv & 3 = 3 ORDER BY id ASC",
    ...
    return ORJSONResponse(
        {
            ...
            "match": {
                ...
                "refs": [{"id": player.id, "name": player.name} for player in match.refs],
...
async def api_get_pool(
    ...
    pool_creator_clan = (
        await clans_repo.fetch_one(id=pool_creator.clan_id) if pool_creator.clan_id is not None else None
    )
