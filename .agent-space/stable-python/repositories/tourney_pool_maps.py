...
async def create(map_id: int, pool_id: int, mods: int, slot: int) -> TourneyPoolMap:
    ...
    select_stmt = (
        select(*READ_PARAMS).where(TourneyPoolMapsTable.map_id == map_id).where(TourneyPoolMapsTable.pool_id == pool_id)
    )
...
async def delete_map_from_pool(pool_id: int, map_id: int) -> TourneyPoolMap | None:
    ...
    select_stmt = (
        select(*READ_PARAMS).where(TourneyPoolMapsTable.pool_id == pool_id).where(TourneyPoolMapsTable.map_id == map_id)
    )
