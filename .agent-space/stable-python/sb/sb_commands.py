...
async def sb_mp_fix(ctx: Context) -> str:
    ...
    res: list[str] = [f"Checking {len([m for m in app.state.sessions.matches if m is not None])} matches..."]
    for match in app.state.sessions.matches:
        ...
        if host is None:
            res.append(f"Found invalid state: Match <{match.id}> {match.name} has no host.")
            next_player = None
            ...
            if next_player is None:
                res.append("Could not find next player, terminating match.")
        ...
        for slot in match.slots:
            ...
            if app.state.sessions.players.get(id=slot.player.id) is None:
                res.append(f"Found invalid state: Match <{match.id}> {match.name} has ghost player {slot.player.name}.")
                slot.player = None
...
sb_mp_room_commands = sb_mp_commands.subcommand(CommandSet("room", "sb mp room commands."))
