...
- Outbox Delivery 日志记录 `event_type`、聚合身份、`schema_version`、`source_position` 和完整 `outbox_payload`，用于直接查看实际事件内容。
...
- `INFO`：进程 ready/stopped、登录成功、房间和回合生命周期、成绩/计算等关键持久状态提交，以及成功的 Outbox Delivery（包含完整事件内容）。
