...
class ScoreAttestation:
    ...
    def __post_init__(self) -> None:
        ...
        if self.checksum is not None and len(self.checksum) != 32:
            raise ValueError("attestation checksum must contain 32 bytes")
...
class PerformanceCalculationInput:
    """Provide complete version-independent facts to an injected PP engine."""

    revision: BeatmapRevisionInfo
    scoreboard: ScoreboardInfo
    mod_set: NormalizedModSet
    score: ScoreSubmission


@dataclass(frozen=True, slots=True)
class PerformanceResult:
    """Store PP produced by one persisted calculation release."""
