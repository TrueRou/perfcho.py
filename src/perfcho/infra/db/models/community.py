...
class Channel(TimestampMixin, DbBase):
    ...
    __table_args__ = (
        ...
        Index("ix_channel_kind_archived", "kind", "archived_at"),
        {"schema": "community"},
    )
    ...
    team_id: Mapped[int | None] = mapped_column(ForeignKey("social.team.id", ondelete="SET NULL"))
    read_permission_id: Mapped[int | None] = mapped_column(ForeignKey("authz.permission.id", ondelete="SET NULL"))
    write_permission_id: Mapped[int | None] = mapped_column(ForeignKey("authz.permission.id", ondelete="SET NULL"))
    manage_permission_id: Mapped[int | None] = mapped_column(ForeignKey("authz.permission.id", ondelete="SET NULL"))
...
class DirectConversation(DbBase):
    ...
    __table_args__ = (
        ...
        Index("ix_direct_conversation_low_account", "low_account_id", "channel_id"),
        Index("ix_direct_conversation_high_account", "high_account_id", "channel_id"),
        {"schema": "community"},
    )

    channel_id: Mapped[int] = mapped_column(ForeignKey("community.channel.id", ondelete="CASCADE"), primary_key=True)
    low_account_id: Mapped[int] = mapped_column(ForeignKey("core.account.id", ondelete="RESTRICT"), nullable=False)
    high_account_id: Mapped[int] = mapped_column(ForeignKey("core.account.id", ondelete="RESTRICT"), nullable=False)
...
class ChannelMembership(BigIntIdentityMixin, CreatedAtMixin, DbBase):
    ...
    __table_args__ = (
        ...
        Index(
            "uq_channel_membership_current",
            "channel_id",
            "account_id",
            unique=True,
            postgresql_where=text("left_at IS NULL"),
        ),
        Index("ix_channel_membership_account", "account_id", "left_at"),
        {"schema": "community"},
    )

    channel_id: Mapped[int] = mapped_column(ForeignKey("community.channel.id", ondelete="RESTRICT"), nullable=False)
...
class Message(BigIntIdentityMixin, CreatedAtMixin, DbBase):
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(
            ...
            ["community.message.channel_id", "community.message.id"],
            name="fk_message_reply_same_channel",
            ondelete="RESTRICT",
        ),
        UniqueConstraint("channel_id", "id"),
        UniqueConstraint("sender_account_id", "client_message_id"),
        Index("ix_message_channel_id_desc", "channel_id", text("id DESC")),
        Index("ix_message_sender_created", "sender_account_id", "created_at"),
        {"schema": "community"},
    )

    channel_id: Mapped[int] = mapped_column(ForeignKey("community.channel.id", ondelete="RESTRICT"), nullable=False)
    sender_account_id: Mapped[int] = mapped_column(ForeignKey("core.account.id", ondelete="RESTRICT"), nullable=False)
...
class ChannelUserState(TimestampMixin, DbBase):
    ...
    __tablename__ = "channel_user_state"
    __table_args__ = (
        ForeignKeyConstraint(
        ...
        Index("ix_channel_user_state_account", "account_id", "hidden", "channel_id"),
        {"schema": "community"},
    )
...
class ChannelReadProjection(TimestampMixin, DbBase):
    ...
    __tablename__ = "channel_read_projection"
    __table_args__ = (
        ...
        Index("ix_channel_read_projection_latest", "latest_message_at", "channel_id"),
        Index("ix_channel_read_projection_position", "source_position"),
        {"schema": "community"},
    )
    ...
    latest_message_id: Mapped[int | None] = mapped_column(ForeignKey("community.message.id", ondelete="SET NULL"))
...
class MessageRevision(BigIntIdentityMixin, CreatedAtMixin, DbBase):
    ...
    __table_args__ = (
        Index("ix_message_revision_message_created", "message_id", "created_at"),
        {"schema": "community"},
    )

    message_id: Mapped[int] = mapped_column(ForeignKey("community.message.id", ondelete="RESTRICT"), nullable=False)
...
class NotificationRecipient(DbBase):
    ...
    notification_id: Mapped[int] = mapped_column(
        ForeignKey("community.notification.id", ondelete="CASCADE"), primary_key=True
    )
    account_id: Mapped[int] = mapped_column(ForeignKey("core.account.id", ondelete="CASCADE"), primary_key=True)
...
class NotificationDispatch(BigIntIdentityMixin, CreatedAtMixin, DbBase):
    ...
    __table_args__ = (
        ...
        Index("ix_notification_dispatch_status", "status", "next_attempt_at"),
        {"schema": "community"},
    )

    notification_id: Mapped[int] = mapped_column(
        ForeignKey("community.notification.id", ondelete="CASCADE"), nullable=False
    )
    account_id: Mapped[int] = mapped_column(ForeignKey("core.account.id", ondelete="CASCADE"), nullable=False)
