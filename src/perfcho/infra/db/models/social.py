...
class Follow(CreatedAtMixin, DbBase):
    ...
    __table_args__ = (
        ...
        Index("ix_follow_target", "target_account_id", "actor_account_id"),
        {"schema": "social"},
    )

    actor_account_id: Mapped[int] = mapped_column(ForeignKey("core.account.id", ondelete="CASCADE"), primary_key=True)
    target_account_id: Mapped[int] = mapped_column(ForeignKey("core.account.id", ondelete="CASCADE"), primary_key=True)
...
class Block(CreatedAtMixin, DbBase):
    ...
    __table_args__ = (
        CheckConstraint("actor_account_id <> target_account_id", name="not_self"),
        Index("ix_block_target", "target_account_id", "actor_account_id"),
        {"schema": "social"},
    )
...
class Team(TimestampMixin, DbBase):
    ...
    __table_args__ = (
        ...
        Index("ix_team_ruleset_archived", "ruleset", "archived_at"),
        {"schema": "social"},
    )
    ...
    flag_asset_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("core.media_asset.id", ondelete="SET NULL"))
...
class TeamMembership(BigIntIdentityMixin, CreatedAtMixin, DbBase):
    ...
    __table_args__ = (
        ...
        Index(
            "uq_team_membership_current_account",
            "account_id",
            unique=True,
            postgresql_where=text("left_at IS NULL"),
        ),
        Index(
            "uq_team_membership_current_owner",
            "team_id",
            unique=True,
            postgresql_where=text("left_at IS NULL AND role = 'owner'"),
        ),
        Index("ix_team_membership_team_active", "team_id", "left_at", "account_id"),
        {"schema": "social"},
    )

    team_id: Mapped[int] = mapped_column(ForeignKey("social.team.id", ondelete="RESTRICT"), nullable=False)
...
class TeamJoinRequest(Uuid7PrimaryKeyMixin, CreatedAtMixin, DbBase):
    ...
    __table_args__ = (
        ...
        Index("ix_team_join_request_candidate", "candidate_account_id", "status"),
        {"schema": "social"},
    )
    ...
    requested_by_id: Mapped[int] = mapped_column(ForeignKey("core.account.id", ondelete="RESTRICT"), nullable=False)
    kind: Mapped[str] = mapped_column(String(16), nullable=False)
    status: Mapped[str] = mapped_column(String(16), nullable=False, default="pending", server_default="pending")
    decided_by_id: Mapped[int | None] = mapped_column(ForeignKey("core.account.id", ondelete="SET NULL"))
...
class AchievementDefinition(TimestampMixin, DbBase):
    ...
    icon_asset_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("core.media_asset.id", ondelete="SET NULL"))
...
class AchievementTranslation(DbBase):
    ...
    __tablename__ = "achievement_translation"
    __table_args__ = ({"schema": "social"},)

    achievement_id: Mapped[int] = mapped_column(
        ForeignKey("social.achievement_definition.id", ondelete="CASCADE"), primary_key=True
    )
...
class AchievementUnlock(CreatedAtMixin, DbBase):
    ...
    __table_args__ = (Index("ix_achievement_unlock_account_created", "account_id", "created_at"), {"schema": "social"})

    account_id: Mapped[int] = mapped_column(ForeignKey("core.account.id", ondelete="RESTRICT"), primary_key=True)
    achievement_id: Mapped[int] = mapped_column(
        ForeignKey("social.achievement_definition.id", ondelete="RESTRICT"), primary_key=True
    )
