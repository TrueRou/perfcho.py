...
class CalculationFormula(Uuid7PrimaryKeyMixin, CreatedAtMixin, DbBase):
    ...
    __table_args__ = (
        ...
        Index("ix_calculation_formula_kind_enabled", "kind", "enabled"),
        {"schema": "scoring"},
    )
...
class CalculationRelease(Uuid7PrimaryKeyMixin, CreatedAtMixin, DbBase):
    ...
    __tablename__ = "calculation_release"
    __table_args__ = (
        UniqueConstraint("formula_id", "ruleset", "version", name="uq_calculation_release_formula_version"),
        Index("ix_calculation_release_active", "formula_id", "ruleset", "active"),
        Index(
            "uq_calculation_release_active_formula_ruleset",
            "formula_id",
            "ruleset",
            unique=True,
    ...
    formula_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("scoring.calculation_formula.id", ondelete="RESTRICT"), nullable=False
    )
    ...
    difficulty_release_id: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey("scoring.calculation_release.id", ondelete="RESTRICT")
    )
...
class BeatmapDifficultyAttribute(BigIntIdentityMixin, CreatedAtMixin, DbBase):
    ...
    __tablename__ = "beatmap_difficulty_attribute"
    __table_args__ = (
    ...
    release_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("scoring.calculation_release.id", ondelete="RESTRICT"), nullable=False
    )
...
class PlayAttempt(Uuid7PrimaryKeyMixin, CreatedAtMixin, DbBase):
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(
            ...
            ["content.beatmap_revision.id", "content.beatmap_revision.beatmap_id"],
            name="fk_play_attempt_revision_beatmap",
            ondelete="RESTRICT",
        ),
        ...
        UniqueConstraint(
            ...
            "mods_acronyms",
            "mods_digest",
            name="uq_play_attempt_score_dimensions",
        ),
        Index("ix_play_attempt_account_started", "account_id", "started_at"),
        Index(
            "ix_play_attempt_account_map_started",
            "account_id",
            "beatmap_id",
            text("started_at DESC"),
            text("id DESC"),
        ),
        Index("ix_play_attempt_revision_started", "beatmap_revision_id", "started_at"),
        Index("ix_play_attempt_ruleset_mods", "ruleset", "mods_digest"),
        Index("ix_play_attempt_status_created", "status", "created_at"),
        {"schema": "scoring"},
    )
...
class PlayAttemptToken(BigIntIdentityMixin, CreatedAtMixin, DbBase):
    ...
    __tablename__ = "play_attempt_token"
    __table_args__ = (
        ...
        Index("ix_play_attempt_token_account_created", "account_id", "created_at"),
        Index("ix_play_attempt_token_expiry", "expires_at"),
        {"schema": "scoring"},
    )
...
class Score(BigIntIdentityMixin, CreatedAtMixin, DbBase):
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(
            ...
            [
                "scoring.play_attempt.id",
                "scoring.play_attempt.account_id",
                "scoring.play_attempt.beatmap_id",
                "scoring.play_attempt.beatmap_revision_id",
                "scoring.play_attempt.ruleset",
                "scoring.play_attempt.mods_details",
                "scoring.play_attempt.mods_acronyms",
                "scoring.play_attempt.mods_digest",
            ],
            name="fk_score_attempt_dimensions",
            ondelete="RESTRICT",
        ),
        UniqueConstraint("attempt_id"),
        UniqueConstraint("online_checksum"),
        Index("ix_score_account_ruleset_ended", "account_id", "ruleset", text("ended_at DESC"), text("id DESC")),
        Index(
            "ix_score_account_map_ended",
            "account_id",
            "beatmap_id",
            text("ended_at DESC"),
            text("id DESC"),
        ),
        Index("ix_score_revision_ruleset", "beatmap_revision_id", "ruleset", "id"),
        Index("ix_score_beatmap_ruleset_mods", "beatmap_id", "ruleset", "mods_digest", "id"),
        {"schema": "scoring"},
    )
...
class ScoreHitStatistic(DbBase):
    ...
    __tablename__ = "score_hit_statistic"
    __table_args__ = (
    ...
    score_id: Mapped[int] = mapped_column(ForeignKey("scoring.score.id", ondelete="CASCADE"), primary_key=True)
...
class ScorePerformance(CreatedAtMixin, DbBase):
    ...
    __table_args__ = (
        ...
        Index("ix_score_performance_release_pp", "release_id", "pp"),
        {"schema": "scoring"},
    )

    score_id: Mapped[int] = mapped_column(ForeignKey("scoring.score.id", ondelete="CASCADE"), primary_key=True)
    release_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("scoring.calculation_release.id", ondelete="RESTRICT"), primary_key=True
    )
    difficulty_attribute_id: Mapped[int] = mapped_column(
        ForeignKey("scoring.beatmap_difficulty_attribute.id", ondelete="RESTRICT"), nullable=False
    )
...
class RankingPolicy(Uuid7PrimaryKeyMixin, DbBase):
    ...
    __tablename__ = "ranking_policy"
    __table_args__ = (
        UniqueConstraint("code"),
        Index("ix_ranking_policy_ruleset_active", "ruleset", "active"),
        {"schema": "scoring"},
    )
...
class ScoreEligibility(TimestampMixin, DbBase):
    ...
    policy_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("scoring.ranking_policy.id", ondelete="CASCADE"), primary_key=True
    )
...
class Replay(CreatedAtMixin, DbBase):
    ...
    __table_args__ = (
        ...
        Index("ix_replay_state_created", "state", "created_at"),
        {"schema": "scoring"},
    )

    score_id: Mapped[int] = mapped_column(ForeignKey("scoring.score.id", ondelete="RESTRICT"), primary_key=True)
...
class ReplayViewEvent(BigIntIdentityMixin, CreatedAtMixin, DbBase):
    ...
    __tablename__ = "replay_view_event"
    __table_args__ = (
    ...
    viewer_account_id: Mapped[int | None] = mapped_column(ForeignKey("core.account.id", ondelete="SET NULL"))


class UserPlayStat(TimestampMixin, DbBase):
    """Projects cumulative factual play statistics for an account and ruleset."""

    __tablename__ = "user_play_stat"
    __table_args__ = (
        CheckConstraint(
        ...
        Index("ix_user_play_stat_score_rank", "ruleset", text("total_score DESC"), "account_id"),
        {"schema": "scoring"},
    )
...
class UserRanking(TimestampMixin, DbBase):
    ...
    __table_args__ = (
        ...
        CheckConstraint("accuracy BETWEEN 0 AND 1", name="accuracy_range"),
        Index("ix_user_ranking_rank", "policy_id", "performance", "account_id"),
        Index("ix_user_ranking_ranked_score", "policy_id", "ranked_score", "account_id"),
        {"schema": "scoring"},
    )
...
class BeatmapFailHistogram(TimestampMixin, DbBase):
    ...
    __tablename__ = "beatmap_fail_histogram"
    __table_args__ = (
...
class UserRankingSnapshot(CreatedAtMixin, DbBase):
    """Stores daily global and country ranking snapshots for accounts."""

    __tablename__ = "user_ranking_snapshot"
    __table_args__ = (
        ...
        Index("ix_user_ranking_snapshot_account_history", "account_id", "policy_id", "snapshot_date"),
        {"schema": "scoring"},
    )
...
class ScoreAttestation(CreatedAtMixin, DbBase):
    ...
    __tablename__ = "score_attestation"
    __table_args__ = (Index("ix_score_attestation_state", "verification_state", "created_at"), {"schema": "scoring"})
