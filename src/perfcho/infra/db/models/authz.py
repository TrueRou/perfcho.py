...
class RolePermission(DbBase):
    ...
    role_id: Mapped[int] = mapped_column(ForeignKey("authz.role.id", ondelete="CASCADE"), primary_key=True)
    permission_id: Mapped[int] = mapped_column(ForeignKey("authz.permission.id", ondelete="CASCADE"), primary_key=True)
...
class AccountRoleGrant(Uuid7PrimaryKeyMixin, CreatedAtMixin, DbBase):
    ...
    __tablename__ = "account_role_grant"
    __table_args__ = (
        CheckConstraint("ends_at IS NULL OR ends_at > starts_at", name="valid_period"),
        Index("ix_account_role_grant_account_active", "account_id", "starts_at", "ends_at"),
        Index("ix_account_role_grant_role_active", "role_id", "starts_at", "ends_at"),
        {"schema": "authz"},
    )

    account_id: Mapped[int] = mapped_column(ForeignKey("core.account.id", ondelete="RESTRICT"), nullable=False)
    role_id: Mapped[int] = mapped_column(ForeignKey("authz.role.id", ondelete="RESTRICT"), nullable=False)
    granted_by_id: Mapped[int | None] = mapped_column(ForeignKey("core.account.id", ondelete="SET NULL"))
...
class AccountPermissionGrant(Uuid7PrimaryKeyMixin, CreatedAtMixin, DbBase):
    ...
    __tablename__ = "account_permission_grant"
    __table_args__ = (
        CheckConstraint("ends_at IS NULL OR ends_at > starts_at", name="valid_period"),
        Index("ix_account_permission_grant_account_active", "account_id", "starts_at", "ends_at"),
        {"schema": "authz"},
    )

    account_id: Mapped[int] = mapped_column(ForeignKey("core.account.id", ondelete="RESTRICT"), nullable=False)
    permission_id: Mapped[int] = mapped_column(ForeignKey("authz.permission.id", ondelete="RESTRICT"), nullable=False)
...
class AccountEntitlementGrant(Uuid7PrimaryKeyMixin, CreatedAtMixin, DbBase):
    ...
    __tablename__ = "account_entitlement_grant"
    __table_args__ = (
        CheckConstraint("ends_at IS NULL OR ends_at > starts_at", name="valid_period"),
        Index("ix_account_entitlement_grant_account_active", "account_id", "starts_at", "ends_at"),
        {"schema": "authz"},
    )

    account_id: Mapped[int] = mapped_column(ForeignKey("core.account.id", ondelete="RESTRICT"), nullable=False)
    entitlement_id: Mapped[int] = mapped_column(
        ForeignKey("authz.entitlement.id", ondelete="RESTRICT"), nullable=False
    )
    granted_by_id: Mapped[int | None] = mapped_column(ForeignKey("core.account.id", ondelete="SET NULL"))
    starts_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    ends_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
