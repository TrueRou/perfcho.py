...
class Room(Uuid7PrimaryKeyMixin, TimestampMixin, DbBase):
    ...
    __table_args__ = (
        ...
        Index(
            "uq_room_active_public_id",
            "public_id",
            unique=True,
            postgresql_where=text("status IN ('open', 'started')"),
        ),
        Index("ix_room_category_status_start", "category", "status", "starts_at"),
        Index("ix_room_creator_created", "creator_account_id", "created_at"),
        {"schema": "multiplayer"},
    )
    ...
    creator_account_id: Mapped[int] = mapped_column(ForeignKey("core.account.id", ondelete="RESTRICT"), nullable=False)
    channel_id: Mapped[int | None] = mapped_column(ForeignKey("community.channel.id", ondelete="SET NULL"))
...
class MultiplayerSession(Uuid7PrimaryKeyMixin, CreatedAtMixin, DbBase):
    ...
    __table_args__ = (
        ...
        Index("ix_session_room_status", "room_id", "status"),
        Index("ix_session_host_status", "host_account_id", "status"),
        {"schema": "multiplayer"},
    )

    room_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("multiplayer.room.id", ondelete="RESTRICT"), nullable=False)
    ordinal: Mapped[int] = mapped_column(Integer, nullable=False)
    host_account_id: Mapped[int | None] = mapped_column(ForeignKey("core.account.id", ondelete="SET NULL"))
...
class RoomParticipant(CreatedAtMixin, DbBase):
    ...
    __table_args__ = (
        ...
        Index("ix_room_participant_account_activity", "account_id", "last_activity_at"),
        Index("ix_room_participant_room_status", "room_id", "status"),
        {"schema": "multiplayer"},
    )

    room_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("multiplayer.room.id", ondelete="RESTRICT"), primary_key=True
    )
...
class SessionPresence(Uuid7PrimaryKeyMixin, CreatedAtMixin, DbBase):
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(
            ["room_id", "account_id"],
            ["multiplayer.room_participant.room_id", "multiplayer.room_participant.account_id"],
            ondelete="RESTRICT",
        ),
        ForeignKeyConstraint(
            ["session_id", "room_id"],
            ["multiplayer.session.id", "multiplayer.session.room_id"],
            name="fk_session_presence_session_room",
            ondelete="RESTRICT",
        ),
        UniqueConstraint("session_id", "account_id", "join_number"),
        Index(
            "uq_session_presence_current",
            "session_id",
            "account_id",
            unique=True,
        ...
        Index(
            "uq_session_presence_account_current",
            "account_id",
            unique=True,
            postgresql_where=text("left_at IS NULL"),
        ),
        Index("ix_session_presence_connection", "connection_session_id", "account_id"),
        Index("ix_session_presence_account", "account_id", "created_at"),
        {"schema": "multiplayer"},
    )
...
class PlaylistItem(Uuid7PrimaryKeyMixin, TimestampMixin, DbBase):
    ...
    __table_args__ = (
        ...
        Index("ix_playlist_item_room_state", "room_id", "state", "client_item_id"),
        {"schema": "multiplayer"},
    )
...
class PlaylistRevision(Uuid7PrimaryKeyMixin, CreatedAtMixin, DbBase):
    ...
    __tablename__ = "playlist_revision"
    __table_args__ = (
        CheckConstraint("revision_number > 0", name="positive_revision"),
        UniqueConstraint("item_id", "revision_number", name="uq_playlist_revision_item_revision"),
        UniqueConstraint("item_id", "configuration_digest", name="uq_playlist_revision_item_digest"),
        Index(
            "uq_playlist_revision_current",
            "item_id",
            unique=True,
            postgresql_where=text("is_current"),
        ),
        Index("ix_playlist_revision_beatmap", "beatmap_revision_id"),
        {"schema": "multiplayer"},
    )

    item_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("multiplayer.playlist_item.id", ondelete="RESTRICT"), nullable=False
    )
    revision_number: Mapped[int] = mapped_column(Integer, nullable=False)
    owner_account_id: Mapped[int] = mapped_column(ForeignKey("core.account.id", ondelete="RESTRICT"), nullable=False)
    beatmap_revision_id: Mapped[int] = mapped_column(
        ForeignKey("content.beatmap_revision.id", ondelete="RESTRICT"), nullable=False
    )
    ...
    ranking_policy_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("scoring.ranking_policy.id", ondelete="RESTRICT"), nullable=False
    )
...
class TournamentPool(Uuid7PrimaryKeyMixin, TimestampMixin, DbBase):
    ...
    __tablename__ = "tournament_pool"
    __table_args__ = (
        UniqueConstraint("namespace", "name_key"),
        Index("ix_tournament_pool_creator_status", "creator_account_id", "status"),
        {"schema": "multiplayer"},
    )
...
class TournamentPoolRevision(Uuid7PrimaryKeyMixin, CreatedAtMixin, DbBase):
    ...
    __tablename__ = "tournament_pool_revision"
    __table_args__ = (
        CheckConstraint("revision_number > 0", name="positive_revision"),
        UniqueConstraint("pool_id", "revision_number", name="uq_tournament_pool_revision_pool_revision"),
        UniqueConstraint("pool_id", "configuration_digest", name="uq_tournament_pool_revision_pool_digest"),
        Index(
            "uq_tournament_pool_revision_current",
            "pool_id",
            unique=True,
            postgresql_where=text("is_current"),
        ),
    ...
    pool_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("multiplayer.tournament_pool.id", ondelete="RESTRICT"), nullable=False
    )
    revision_number: Mapped[int] = mapped_column(Integer, nullable=False)
    created_by_id: Mapped[int] = mapped_column(ForeignKey("core.account.id", ondelete="RESTRICT"), nullable=False)
    configuration_digest: Mapped[bytes] = mapped_column(LargeBinary(32), nullable=False)
...
class TournamentPoolItem(Uuid7PrimaryKeyMixin, DbBase):
    ...
    __tablename__ = "tournament_pool_item"
    __table_args__ = (
        ...
        UniqueConstraint(
            ...
            "mod_bucket",
            "slot_number",
            name="uq_tournament_pool_item_revision_bucket_slot",
        ),
        UniqueConstraint(
            "revision_id",
            "beatmap_revision_id",
            "mods_digest",
            name="uq_tournament_pool_item_revision_map_mods",
        ),
        Index("ix_tournament_pool_item_beatmap", "beatmap_revision_id"),
        {"schema": "multiplayer"},
    )

    revision_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("multiplayer.tournament_pool_revision.id", ondelete="CASCADE"), nullable=False
    )
    beatmap_revision_id: Mapped[int] = mapped_column(
        ForeignKey("content.beatmap_revision.id", ondelete="RESTRICT"), nullable=False
    )
...
class Round(Uuid7PrimaryKeyMixin, CreatedAtMixin, DbBase):
    ...
    __table_args__ = (
        ...
        Index(
            "uq_round_session_active",
            "session_id",
            unique=True,
            postgresql_where=text("status = 'in_progress'"),
        ),
        Index("ix_round_playlist_revision", "playlist_revision_id"),
        {"schema": "multiplayer"},
    )

    session_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("multiplayer.session.id", ondelete="RESTRICT"), nullable=False
    )
    round_number: Mapped[int] = mapped_column(Integer, nullable=False)
    playlist_revision_id: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey("multiplayer.playlist_revision.id", ondelete="RESTRICT")
    )
    tournament_pool_item_id: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey("multiplayer.tournament_pool_item.id", ondelete="RESTRICT")
    )
...
class RoundParticipant(DbBase):
    ...
    __table_args__ = (
        ...
        Index(
            "uq_round_participant_slot",
            "round_id",
            "slot_number",
            unique=True,
            postgresql_where=text("slot_number IS NOT NULL"),
        ),
        Index("ix_round_participant_account", "account_id", "round_id"),
        {"schema": "multiplayer"},
    )

    round_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("multiplayer.round.id", ondelete="RESTRICT"), primary_key=True
    )
    account_id: Mapped[int] = mapped_column(ForeignKey("core.account.id", ondelete="RESTRICT"), primary_key=True)
    presence_id: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey("multiplayer.session_presence.id", ondelete="SET NULL")
    )
...
class MultiplayerAttempt(Uuid7PrimaryKeyMixin, CreatedAtMixin, DbBase):
    ...
    round_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("multiplayer.round.id", ondelete="RESTRICT"))
    playlist_revision_id: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey("multiplayer.playlist_revision.id", ondelete="RESTRICT")
    )
    play_attempt_id: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey("scoring.play_attempt.id", ondelete="RESTRICT")
    )
...
class MultiplayerEvent(BigIntIdentityMixin, CreatedAtMixin, DbBase):
    ...
    session_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("multiplayer.session.id", ondelete="RESTRICT"))
...
class RoundResult(Uuid7PrimaryKeyMixin, CreatedAtMixin, DbBase):
    ...
    __table_args__ = (
        ...
        Index(
            "uq_round_result_team",
            "round_id",
            "team_number",
            unique=True,
            postgresql_where=text("team_number IS NOT NULL"),
        ),
        Index("ix_round_result_rank", "round_id", "rank"),
        {"schema": "multiplayer"},
    )

    round_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("multiplayer.round.id", ondelete="RESTRICT"), nullable=False
    )
...
class SessionStanding(TimestampMixin, DbBase):
    ...
    __tablename__ = "session_standing"
    __table_args__ = (
        CheckConstraint("points >= 0 AND version >= 0", name="nonnegative_values"),
        Index("ix_session_standing_rank", "session_id", "points"),
        {"schema": "multiplayer"},
    )

    session_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("multiplayer.session.id", ondelete="CASCADE"), primary_key=True
    )
...
class SessionPoolBinding(Uuid7PrimaryKeyMixin, CreatedAtMixin, DbBase):
    ...
    pool_revision_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("multiplayer.tournament_pool_revision.id", ondelete="RESTRICT"), nullable=False
    )
    actor_account_id: Mapped[int] = mapped_column(ForeignKey("core.account.id", ondelete="RESTRICT"), nullable=False)
...
class MatchmakingQueue(Uuid7PrimaryKeyMixin, TimestampMixin, DbBase):
    ...
    __table_args__ = (
        ...
        Index("ix_matchmaking_queue_active_ruleset", "active", "ruleset"),
        {"schema": "multiplayer"},
    )
    ...
    pool_revision_id: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey("multiplayer.tournament_pool_revision.id", ondelete="SET NULL")
    )
...
class MatchmakingGroup(Uuid7PrimaryKeyMixin, CreatedAtMixin, DbBase):
    ...
    __tablename__ = "matchmaking_group"
    __table_args__ = (
        Index("ix_matchmaking_group_queue_status", "queue_id", "status", "created_at"),
        Index("ix_matchmaking_group_owner", "owner_account_id", "status"),
        {"schema": "multiplayer"},
    )

    queue_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("multiplayer.matchmaking_queue.id", ondelete="RESTRICT"), nullable=False
    )
    owner_account_id: Mapped[int] = mapped_column(ForeignKey("core.account.id", ondelete="RESTRICT"), nullable=False)
...
class MatchmakingGroupMember(CreatedAtMixin, DbBase):
    ...
    __tablename__ = "matchmaking_group_member"
    __table_args__ = (
        CheckConstraint("seat_number >= 0", name="nonnegative_seat"),
        UniqueConstraint("group_id", "seat_number"),
        Index("ix_matchmaking_group_member_account", "account_id", "group_id"),
        {"schema": "multiplayer"},
    )

    group_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("multiplayer.matchmaking_group.id", ondelete="CASCADE"), primary_key=True
    )
    account_id: Mapped[int] = mapped_column(ForeignKey("core.account.id", ondelete="RESTRICT"), primary_key=True)
...
class MatchmakingTicket(Uuid7PrimaryKeyMixin, CreatedAtMixin, DbBase):
    ...
    __tablename__ = "matchmaking_ticket"
    __table_args__ = (
        CheckConstraint("expires_at > created_at", name="valid_period"),
        Index(
            "uq_matchmaking_ticket_queued_group",
            "group_id",
            unique=True,
            postgresql_where=text("state = 'queued'"),
        ),
        Index(
            "ix_matchmaking_ticket_queue_order",
            "queue_id",
            "priority",
            "created_at",
            postgresql_where=text("state = 'queued'"),
    ...
    group_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("multiplayer.matchmaking_group.id", ondelete="RESTRICT"), nullable=False
    )
...
class MatchmakingAssignment(Uuid7PrimaryKeyMixin, CreatedAtMixin, DbBase):
    ...
    __tablename__ = "matchmaking_assignment"
    __table_args__ = (
        CheckConstraint("accept_deadline > created_at", name="valid_period"),
        UniqueConstraint("session_id"),
        Index("ix_matchmaking_assignment_deadline", "status", "accept_deadline"),
        {"schema": "multiplayer"},
    )
...
class MatchmakingAssignmentMember(DbBase):
    ...
    __tablename__ = "matchmaking_assignment_member"
    __table_args__ = (
        Index("ix_matchmaking_assignment_member_account", "account_id", "accept_state"),
        {"schema": "multiplayer"},
    )

    assignment_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("multiplayer.matchmaking_assignment.id", ondelete="CASCADE"), primary_key=True
    )
    account_id: Mapped[int] = mapped_column(ForeignKey("core.account.id", ondelete="RESTRICT"), primary_key=True)
    group_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("multiplayer.matchmaking_group.id", ondelete="RESTRICT"), nullable=False
...
class MatchmakingRating(TimestampMixin, DbBase):
    ...
    __tablename__ = "matchmaking_rating"
    __table_args__ = (
        CheckConstraint(
            "rating > 0 AND deviation >= 0 AND volatility >= 0 AND placement_count >= 0 AND version >= 0",
            name="value_ranges",
        ),
        Index("ix_matchmaking_rating_rank", "queue_id", "season_key", "rating", "account_id"),
        {"schema": "multiplayer"},
    )

    account_id: Mapped[int] = mapped_column(ForeignKey("core.account.id", ondelete="CASCADE"), primary_key=True)
    queue_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("multiplayer.matchmaking_queue.id", ondelete="CASCADE"), primary_key=True
    )
...
class MatchmakingRatingChange(BigIntIdentityMixin, CreatedAtMixin, DbBase):
    ...
    __tablename__ = "matchmaking_rating_change"
    __table_args__ = (
        UniqueConstraint("assignment_id", "account_id", "algorithm_version"),
        Index("ix_matchmaking_rating_change_account", "account_id", "created_at"),
        {"schema": "multiplayer"},
    )

    assignment_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("multiplayer.matchmaking_assignment.id", ondelete="RESTRICT"), nullable=False
    )
    account_id: Mapped[int] = mapped_column(ForeignKey("core.account.id", ondelete="RESTRICT"), nullable=False)
    queue_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("multiplayer.matchmaking_queue.id", ondelete="RESTRICT"), nullable=False
...
class DailyChallenge(Uuid7PrimaryKeyMixin, CreatedAtMixin, DbBase):
    ...
    __table_args__ = (
        ...
        Index("ix_daily_challenge_status_start", "status", "starts_at"),
        {"schema": "multiplayer"},
    )
    ...
    playlist_revision_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("multiplayer.playlist_revision.id", ondelete="RESTRICT"), nullable=False
    )
...
class DailyChallengeCompletion(CreatedAtMixin, DbBase):
    ...
    __tablename__ = "daily_challenge_completion"
    __table_args__ = (
        ...
        UniqueConstraint("score_id"),
        Index("ix_daily_challenge_completion_account", "account_id", "created_at"),
        Index("ix_daily_challenge_completion_rank", "challenge_id", "rank"),
        {"schema": "multiplayer"},
    )

    challenge_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("multiplayer.daily_challenge.id", ondelete="RESTRICT"), primary_key=True
    )
    account_id: Mapped[int] = mapped_column(ForeignKey("core.account.id", ondelete="RESTRICT"), primary_key=True)
    attempt_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("multiplayer.attempt.id", ondelete="RESTRICT"), nullable=False
    )
...
class PlaylistItemUserSummary(TimestampMixin, DbBase):
    ...
    __tablename__ = "playlist_item_user_summary"
    __table_args__ = (
    ...
    playlist_item_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("multiplayer.playlist_item.id", ondelete="CASCADE"), primary_key=True
    )
    account_id: Mapped[int] = mapped_column(ForeignKey("core.account.id", ondelete="CASCADE"), primary_key=True)
    attempt_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0, server_default="0")
    completion_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0, server_default="0")
    best_score_id: Mapped[int | None] = mapped_column(ForeignKey("scoring.score.id", ondelete="SET NULL"))
...
class RoomUserSummary(TimestampMixin, DbBase):
    ...
    __tablename__ = "room_user_summary"
    __table_args__ = (
        CheckConstraint("attempt_count >= 0 AND completion_count >= 0 AND total_score >= 0", name="nonnegative_values"),
        Index("ix_room_user_summary_rank", "room_id", "total_score", "account_id"),
        {"schema": "multiplayer"},
    )

    room_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("multiplayer.room.id", ondelete="CASCADE"), primary_key=True)
    account_id: Mapped[int] = mapped_column(ForeignKey("core.account.id", ondelete="CASCADE"), primary_key=True)
    attempt_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0, server_default="0")
    completion_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0, server_default="0")
...
class DailyChallengeUserSummary(TimestampMixin, DbBase):
    ...
    __tablename__ = "daily_challenge_user_summary"
    __table_args__ = (
        CheckConstraint(
